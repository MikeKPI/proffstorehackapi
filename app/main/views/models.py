...
username = "neo4j"
password = os.environ.get("NEO4J_PASS", None)

if password:
    authenticate(neo_url.strip("http://"), username, password)

graph = Graph(neo_url + '/db/data/')
...
class User:
    ...
    def add_project(self, title, tags, text):
        user = self.find()
        post = Node(
            "Post",
            id=str(uuid.uuid4()),
